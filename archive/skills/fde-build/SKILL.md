...
## Post-feature cleanup pass (after it works, before review)

When the feature runs locally but the diff feels messy, run a **cleanup pass only** — not a redesign.

1. Inspect files touched by this slice.
2. Find duplicated mechanics: repeated API calls, parsing, validation, or error handling.
3. Extract the smallest reusable service-layer module. Routes/actions keep **what** happens; services keep **how**.
4. Do not change user-facing behavior. Do not rename unrelated symbols.
5. Re-run the same tests/typechecks that proved the feature worked.

Log in `decisions.md`: what was duplicated and what was extracted.

## Source-first integrations

Before coding against a vendor SDK, framework, or internal platform API:

1. Search local reference source under `reference/repos/...` if the customer provided it, or clone into that path with approval.
2. Record which files/functions you used in `decisions.md`.
3. Implement the minimal service function and one caller — not the whole integration surface.

If the agent invents an API that does not exist, stop and search source; do not add a replacement package without a scope conversation.

## Surgical change discipline

- Every changed line should trace to the task in `decisions.md`.
- Do not "improve" adjacent code, comments, or formatting.
- Remove only orphans **your** change created — not pre-existing dead code unless asked.
- If you wrote 200 lines and 50 would do, rewrite before review.
